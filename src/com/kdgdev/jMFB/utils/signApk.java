...
public class signApk {
    ...
    private static void writeSignatureFile(Manifest manifest, SignatureOutputStream out)
        ...
        for (Map.Entry<String, Attributes> entry : entries.entrySet()) {
            ...
            print.print("\r\n");
            print.flush();
            if (VerboseMode) System.err.println(entry.getKey());
...
//        DataInputStream.read(zipdata, ZipFileSize-22, 22);
            ...
            int BlockLenght;
            if (VerboseMode) System.err.println(" Signing Whole File");
            long Percent = 0;
            long NewPercent = 0;
            long TotalRead = 0;
...
//        FileOutputStream ZipFileStreamOut = new FileOutputStream(ZipFileName);
    ...
    private static void copyFiles(Manifest manifest,
        ...
        for (String name : names) {
            ...
            if (VerboseMode) System.err.println(name);
    ...
    public void signBuildFile(Boolean signWholeFile, String pk509, String pk8, String InputFl, String OutputFl) {
        JarFile inputJar = null;
        JarOutputStream outputJar = null;
        FileOutputStream outputFile = null;
        try {
            X509Certificate publicKey = readPublicKey(new File(pk509));
            long timestamp = publicKey.getNotBefore().getTime() + 3600L * 1000;
            PrivateKey privateKey = readPrivateKey(new File(pk8));
            inputJar = new JarFile(new File(InputFl), false);
            OutputStream outputStream = null;
            //            if (signWholeFile) {
//                outputStream = new ByteArrayOutputStream();
//            } else {
//                outputStream = outputFile = new FileOutputStream(args[argstart+3]);
            outputStream = outputFile = new FileOutputStream(OutputFl);
//            }
            ...
            if (signWholeFile) {
                signWholeOutputFile(OutputFl, publicKey, privateKey);
            }
        } catch (Exception e) {
